...
public class Derbyhelper {
    private DerbyDAOFactory daoFactory;
    private ProductoDAO productoDAO;
    private ClienteDAO clienteDAO;
    private FacturaDAO facturaDAO;
    private Factura_ProductoDAO facturaProductoDAO;

    public Derbyhelper(DerbyDAOFactory daoFactory) {
    ...
    public void createTableCliente() {
        String createTableSQL = "CREATE TABLE Cliente (idCliente INT, nombre VARCHAR (500), email VARCHAR (150), PRIMARY KEY(idCliente))";
        executeStatement(createTableSQL, "Tabla Cliente creada con éxito en Derby.");
    }

    public void createTableFacturaProducto() {
        String createTableSQL = "CREATE TABLE Factura_Producto (idFactura INT, idProducto INT, cantidad INT, PRIMARY KEY(idFactura, idProducto))";
        executeStatement(createTableSQL, "Tabla Factura_Producto creada con éxito en Derby.");
    }

    public void createTableFactura() {
        String createTableSQL = "CREATE TABLE Factura (idFactura INT, idCliente INT, PRIMARY KEY(idFactura))";
        executeStatement(createTableSQL, "Tabla Factura creada con éxito en Derby.");
    }

    public void createTableProducto() {
        String createTableSQL = "CREATE TABLE Producto (idProducto INT, nombre VARCHAR (45), valor FLOAT, PRIMARY KEY(idProducto))";
        executeStatement(createTableSQL, "Tabla Producto creada con éxito en Derby.");
    }
    ...
    public void loadClientesFromCSV(String csvFilePath) {
             ...
             CSVParser parser = new CSVParser(reader, CSVFormat.DEFAULT.withHeader("idCliente", "nombre", "email").withIgnoreHeaderCase().withTrim());)
        {
            ...
            System.out.println("Datos de clientes  cargados desde CSV.");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void loadFacturas_ProductosFromCSV(String csvFilePath) {
        try (FileReader reader = new FileReader(csvFilePath);
            ...
            System.out.println("Datos de factura_productos  cargados desde CSV.");
    ...
    public void loadFacturasFromCSV(String csvFilePath) {
        try (FileReader reader = new FileReader(csvFilePath);
            ...
            System.out.println("Datos de facturas  cargados desde CSV.");
    ...
    public void loadProductosFromCSV(String csvFilePath) {
        try (FileReader reader = new FileReader(csvFilePath);
            ...
            System.out.println("Datos de Productos  cargados desde CSV.");
